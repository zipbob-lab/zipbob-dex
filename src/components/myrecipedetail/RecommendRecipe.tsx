...
const RecommendRecipe = () => {
  ...
  const fetchRamdomRecipe = async () => {
    const { data, error } = await browserClient.from("MY_RECIPE_TABLE").select("*").not('recipe_seq', 'is', null).order("recipe_seq", { ascending: false }).limit(4);
  ...
  return (
    <div className="w-full rounded-[2.5rem] bg-transparent">
      ...
      <div className="mt-[1rem] flex gap-[1rem] overflow-x-auto">
        {posts?.map((post) => <RecipeCard2 key={post.id} post={post} />)}
      </div>
    </div>
  );
};
