...
const RecipeCard = ({ post, isEditMode = false, onDelete }: ExtendedRecipeCardProps) => {
  ...
  return (
    <div
      className="flex w-[10rem] cursor-pointer flex-col rounded-[1.25rem] bg-white p-[0.75rem] shadow-[0px_4px_20px_0px_rgba(154,130,102,0.1)] lg:w-[15.25rem] lg:p-[1rem]"
      onClick={() => router.push(`/myrecipedetail/${post.post_id}`)}
    >
      <div className="relative h-[8.5rem] w-[8.5rem] overflow-hidden lg:h-[13.25rem] lg:w-[13.25rem]">
        <Image
          src={post.recipe_img_done || DefaultImage}
          alt="레시피 사진"
          fill
          sizes="(min-width: 1440px) 13.25rem, 8.5rem"
      ...
      <div className="overflow-hidden text-start">
        ...
        <p className="mb-2 mt-1 h-[1.0125rem] text-r-body-12 text-gray-500 lg:h-[1.011875rem] lg:text-body-13 xl:mb-3">
          {nickname}
        </p>
      </div>
      <div className="flex justify-between">
        <div className="flex items-center">
          <Image src={FireFilledIcon} alt="레시피 난이도" className="h-[1rem] w-[1rem] lg:h-[1.25rem] lg:w-[1.25rem]" />
          <Image
